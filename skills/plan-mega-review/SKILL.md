...
> **Provenance.** Adapted from Garry Tan's `plan-mega-review` (upstream version
...
**Review only.** This skill reviews a plan with maximum rigor and proposes
changes. It does not edit code, run migrations, or write files — the one
exception is adding an item to your backlog, and only when you explicitly approve it.

**Stack-agnostic.** The examples below use Ruby/Rails idioms (ActiveRecord,
`rescue`, `Rails.logger`, `Gemfile`, `rake`) because that is the upstream
author's stack. Read them as **illustrations** and map each to yours: exception
classes → your language's errors; ActiveRecord N+1 → your ORM's eager-loading;
gems/npm → your package manager; rake → your task runner; `Gemfile.lock` → your
lockfile. The rigor is the point, not the framework.
...
7. Everything deferred must be written down. Vague intentions are lies. Capture it in your backlog (TODOS.md, issue tracker, etc.) or it doesn't exist.
...
```
git log --oneline -30                 # recent history (any VCS)
git diff <base-branch> --stat         # what's already changed
git stash list                        # any stashed work
# scan for leftover markers in your source file types, e.g.:
grep -rIl "TODO\|FIXME\|HACK\|XXX" .
# list recently touched source files relative to your lockfile/build file
```
Then read whatever project guidance files exist (CLAUDE.md, AGENTS.md, GEMINI.md,
CONTRIBUTING, README), your backlog, and any existing architecture docs. Map:
...
* Logging only `error.message` is insufficient — log enough to diagnose (the operation, the error class/name, a correlation/request id). But **redact secrets and PII**: never log raw arguments, tokens, credentials, or user content. Prefer structured, low-cardinality fields.
...
* Secrets and credentials. New secrets? Held in a secrets manager or env vars (per your platform's convention), never hardcoded? Rotatable?
* Dependency risk. New third-party dependencies (gems, npm, pip, modules, etc.)? Security track record?
...
Evaluate:
* N+1 queries. For every new ORM association/relation traversal: is there eager-loading (includes/preload/join, per your ORM)?
...
Evaluate:
* Logging. Meaningful structured log lines at key decision points and failure paths — redacted and low-cardinality. Do not log every branch or raw inputs; over-logging is its own defect.
...
* Admin tooling. New operational tasks that need admin UI or task-runner scripts (rake, make, npm scripts, etc.)?
...
* Ecosystem fit. Aligns with your language/framework ecosystem's direction?
...
Present each potential deferred item as its own individual AskUserQuestion. Never batch them — one per question. Never silently skip this step.
