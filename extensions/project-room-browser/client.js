...
async function load(pathOverride) {
    ...
    const r = await fetch("/api/room?path=" + encodeURIComponent(p));
...
async function renderTeams() {
    ...
    $("#sweepbtn").onclick = async () => {
        const r = await fetch("/api/teams/sweep");
        const j = await r.json();
        if (!j.ok) {
...
async function showFile(rel) {
    ...
    try {
        ...
        if (f.kind === "image") {
            ...
            body =
                '<img class="preview" alt="' + h(rel) + '" src="/api/raw?rel=' + encodeURIComponent(rel) + '">';
...
async function renderPicker(errMsg, lastTried) {
    ...
    try {
        ...
        data = await (await fetch("/api/browse" + dir)).json();
    } catch (e) {
